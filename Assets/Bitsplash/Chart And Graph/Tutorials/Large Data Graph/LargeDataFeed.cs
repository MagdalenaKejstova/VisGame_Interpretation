...
public partial class LargeDataFeed : MonoBehaviour, IComparer<DoubleVector2>
{
    ...
    void LoadWithoutDownSampling(int start, int end, bool isBezier)
    {
    ...
    void LoadWithDownSampling(int start, int end, bool isBezier)
    {
        int total = end - start;

        if (DownSampleToPoints >= total)
        {
            LoadWithoutDownSampling(start, end, isBezier);
            return;
        }
        ...
        for (int i = 0; i < DownSampleToPoints; i++)
        {
            ...
            if (divide > 0.0)
            {
                ...
                if (isBezier)
                {
                    if (i == 0)
                    {
                        graph.DataSource.SetCurveInitialPoint(Category, x, y);
                    }
                    else
                    {
                        graph.DataSource.AddLinearCurveToCategory(Category, new DoubleVector2(x, y));
                    }
                }
                else
                {
    ...
    void LoadPage(double pagePosition)
    {
        ...
        if (graph != null)
        {
            ...
            var isBezier = graph.DataSource.IsBezierCurve(Category);
            mStart = start;
            if (DownSampleToPoints <= 0)
                LoadWithoutDownSampling(start, end, isBezier);
            else
                LoadWithDownSampling(start, end, isBezier);
